    ...
    <div style={{minHeight:"100vh",background:"#FAFAF8",color:"#2D2420",fontFamily:"'Nunito',sans-serif",maxWidth:520,margin:"0 auto"}}>
      ...
      <Modal open={showDrink} onClose={()=>setShowDrink(false)}>
        ...
        <div style={{marginTop:12,display:"flex",flexDirection:"column",gap:8}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}><div><label style={fl}>Price</label><input type="number" step="0.01" value={dF.price} onChange={e=>setDF(f=>({...f,price:e.target.value}))} placeholder="$" style={fi} /></div><div><label style={fl}>Date</label><input type="date" value={dF.date} onChange={e=>setDF(f=>({...f,date:e.target.value}))} style={fi} /></div></div>
          <div><label style={fl}>Rating (0–100)</label><ScoreInput value={dF.rating} onChange={r=>setDF(f=>({...f,rating:r}))} /></div>
      ...
      <Modal open={!!showEdit} onClose={()=>{setShowEdit(null);setEF(null)}}>
        {eF&&<div>
          ...
          <div style={{marginTop:12,display:"flex",flexDirection:"column",gap:8}}>
            ...
            {eF.kind==="drink"&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}><div><label style={fl}>Price</label><input type="number" step="0.01" value={eF.price} onChange={e=>setEF(f=>({...f,price:e.target.value}))} style={fi} /></div><div><label style={fl}>Date</label><input type="date" value={eF.date} onChange={e=>setEF(f=>({...f,date:e.target.value}))} style={fi} /></div></div>}
            {eF.kind==="drink"&&<div><label style={fl}>Rating (0–100)</label><ScoreInput value={eF.rating} onChange={r=>setEF(f=>({...f,rating:r}))} /></div>}
